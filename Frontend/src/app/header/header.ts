...
export class Header {
  ...
  constructor(
    ...
    this.carrelloCount = carrello.reduce((totale, prodotto) => totale + (prodotto.quantita), 0);
    });
  ...
  @HostListener('document:click') onDocumentClick() { this.showSignInMenu = false; this.showSuggestions = false; this.searchQuery = ''; }
  ...
  onSearchInput(event: any) { const query = event.target.value; this.searchQuery = query; this.searchSubject.next(query); if(this.searchQuery.length == 0){this.showSuggestions = false;} }
